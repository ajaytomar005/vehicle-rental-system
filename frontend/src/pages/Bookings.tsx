...
export default function Bookings() {
  ...
  useEffect(() => {
    api
      .get<Page<Booking>>('/bookings/me', { params: { size: 20 } })
        ...
        <div className="mt-8 space-y-3">
          {data.content.map((b, i) => (
            ...
            >
              <Link
                to={`/bookings/${b.id}`}
                className="flex items-center gap-4 rounded-2xl border border-white/8 bg-ink-800/40 p-4 transition hover:border-white/20"
              >
                <img
