      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-3xl p-8">
        ...
        {booking.status === 'PENDING_PAYMENT' && (
          <div className="mt-6 flex gap-3">
            <button onClick={simulatePayment} disabled={paying} className="flex-1 rounded-xl bg-gradient-to-r from-ember-500 to-ember-400 py-3 text-sm font-semibold text-ink-950 disabled:opacity-50">
              {paying ? 'Processing…' : 'Pay now'}
            </button>
            <button onClick={cancelBooking} className="rounded-xl border border-white/15 px-5 py-3 text-sm font-medium text-white/70 hover:border-white/30">
              Cancel
            </button>
          </div>
        )}
